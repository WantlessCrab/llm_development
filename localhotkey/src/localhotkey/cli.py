...
def validate_config(config: dict[str, Any]) -> None:
    ...
    for int_key in ("restore_delay_ms", "clipboard_settle_ms", "paste_trigger_delay_ms"):
        default_value = 450 if int_key == "restore_delay_ms" else 160 if int_key == "paste_trigger_delay_ms" else 80
        value = backend.get(int_key, default_value)
...
def backend_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "restore_delay_ms": backend.get("restore_delay_ms", 450),
        "clipboard_settle_ms": backend.get("clipboard_settle_ms", 80),
        "paste_trigger_delay_ms": backend.get("paste_trigger_delay_ms", 160),
    }
...
def set_clipboard(text: str, loops: int | None = None) -> None:
    # In X11, xclip may remain alive as the clipboard selection owner.
    # Do not wait for it to exit; feed stdin, close it, and let it serve the selection.
    argv = ["xclip", "-selection", "clipboard", "-in"]
    if loops is not None:
        argv.extend(["-loops", str(loops)])
    ...
    try:
        ...
        raise LocalHotkeyError("missing executable: xclip") from exc
...
def send_paste_key(paste_key: str) -> None:
    # Hotkey-triggered execution can leave Ctrl/Shift logically active while sxhkd
    # launches this command. Release common modifiers before sending the paste chord.
    run_process(
        [
            "xdotool",
            "keyup",
    ...
    run_process(["xdotool", "key", "--delay", "20", "--clearmodifiers", paste_key])
...
def execute_wrap(wrapper_name: str, config: dict[str, Any]) -> None:
    ...
    # Keep the wrapped clipboard owner alive until restore replaces it.
    # Do not use xclip -loops 1: clipboard managers can consume the single request
    # before the target application receives Ctrl+V.
    set_clipboard(wrapped)
    time.sleep(backend["clipboard_settle_ms"] / 1000)
    time.sleep(backend["paste_trigger_delay_ms"] / 1000)
    send_paste_key(backend["paste_key"])

    if backend["restore_clipboard"]:
        time.sleep(backend["restore_delay_ms"] / 1000)
        # Restored clipboard should persist until the user/app replaces it.
...
def cmd_status(_: argparse.Namespace) -> int:
    ...
    if backend:
        ...
        print(f"paste_trigger_delay_ms={backend.get('paste_trigger_delay_ms')}")
